...
mostrar_submenu_estado:
    ...
    mov ah, 01h
    int 21h
    mov bl, al          ; Guardamos lo que el usuario digitÃ³ en BL
    ...
    ; 6. Redirigir segÃºn lo que eligiÃ³ el usuario
    cmp bl, '1'
    je hacer_activacion
    cmp bl, '2'
    je hacer_desactivacion
    
    ; Si digitÃ³ otra cosa (ej. un 5 o una letra)
    lea dx, mensaje_error
    mov ah, 09h
    int 21h
    call esperar_tecla
...
hacer_activacion:
    cmp al, 1           ; Verificamos si YA estÃ¡ activa
    je error_ya_activa
...
hacer_desactivacion:
    cmp al, 0           ; Verificamos si YA estÃ¡ inactiva
...
opcion_reporte proc
    ...
    cmp num_cuentas, 0
    ja iniciar_reporte      ; Si hay mÃ¡s de 0, iniciamos
...
revisar_saldo:
    ...
    mov rep_saldo_max, ax   ; Â¡Nuevo campeon maximo!

evaluar_minimo:
    cmp ax, rep_saldo_min
    jae siguiente_cuenta_rep; Si es mayor o igual, ignoramos
    mov rep_saldo_min, ax   ; Â¡Nuevo campeon minimo!
